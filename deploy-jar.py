...
host_dic = {
    '192.168.17.131': ['root', '123456', 22, "192.168.17.131"]
}
...
def run_cmd(ssh_client, cmd):
    ...
    if error_msg:
        print("run_cmd error: " + str(error_msg))
...
def back_old_jar(ssh_client, sftp,  parent_dir):
    ...
    # 备份旧文件
    old_files = parent_dir + "tmri* "  + parent_dir + "application* "
...
def deploy(host, host_info, local_dir, remote_dir, jar_name,prameter):
    """
    部署nziot-api
    :param host:
    :param port:
    ...
    run_jar(ssh_client, remote_dir, jar_name+" "+prameter, cfg_name)
...
if __name__ == "__main__":
    ...
    for host, host_info in host_dic.items():
        deploy(host, host_info, local_dir, remote_dir, jar_name,prameter)
